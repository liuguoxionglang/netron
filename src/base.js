...
base.Utility = class {
    ...
    static multiply(a, b, unsigned) {
        ...
        if (a.compare(base.Int64.power24) < 0 && b.compare(base.Int64.power24) < 0) {
            return base.Utility.from(a.toNumber() * b.toNumber(), unsigned);
        }
        const a48 = a.high >>> 16;
        const a32 = a.high & 0xFFFF;
        const a16 = a.low >>> 16;
    ...
    static divide(a, b, unsigned) {
        ...
        while (remainder.compare(b) >= 0) {
            ...
            let approxRes = base.Utility.from(approx);
            let approxRem = approxRes.multiply(b);
            while (approxRem.isNegative || approxRem.compare(remainder) > 0) {
                approx -= delta;
                approxRes = base.Utility.from(approx, unsigned);
                approxRem = approxRes.multiply(b);
            }
            if (approxRes.zero) {
                approxRes = base.Int64.one;
            }
            result = result.add(approxRes);
            remainder = remainder.subtract(approxRem);
        }
        return result;
    }

    static text(value, unsigned, radix) {
        const power = base.Utility.from(Math.pow(radix, 6), unsigned);
        let remainder = value;
        let result = '';
        for (;;) {
    ...
    static from(value, unsigned) {
        ...
        if (value < 0) {
            return base.Utility.from(-value, unsigned).negate();
        }
        return base.Utility._create((value % 4294967296) | 0, (value / 4294967296), unsigned);
    }
